...
def process(ch, method, properties, body):
    ...
    queue_wait_s = round(time.time() - published_at, 4) if published_at else None

    print(f"[recibido] task_id={task_id} limit={limit} queue_wait={queue_wait_s}s", flush=True)
...
def main():
    ...
    while True:
        try:
            ...
            channel.basic_qos(prefetch_count=2)  # 2 tareas en paralelo por worker
